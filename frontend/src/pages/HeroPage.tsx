...
// Scroll-Velocity Linked Marquee Component
const ScrollMarquee: React.FC = () => {
  ...
  const baseSpeed = 0.08; // Base constant speed
...
export const HeroPage: React.FC = () => {
  ...
  // Scroll target progress trackers
  ...
  // Exit transformations for stack layer effects (scales, fades, blurs and slightly translates)
  const springScaleHeroExit = useSpring(useTransform(heroExitProgress, [0, 1], [1, 0.9]), springConfig);
  const springOpacityHeroExit = useSpring(useTransform(heroExitProgress, [0, 1], [1, 0.15]), springConfig);
  const springBlurHeroExit = useSpring(useTransform(heroExitProgress, [0, 1], ["blur(0px)", "blur(12px)"]), springConfig);
  const springYHeroExit = useSpring(useTransform(heroExitProgress, [0, 1], [0, -60]), springConfig);

  const springScaleHowItWorksExit = useSpring(useTransform(howItWorksExitProgress, [0, 1], [1, 0.92]), springConfig);
  const springOpacityHowItWorksExit = useSpring(useTransform(howItWorksExitProgress, [0, 1], [1, 0.25]), springConfig);
  const springBlurHowItWorksExit = useSpring(useTransform(howItWorksExitProgress, [0, 1], ["blur(0px)", "blur(10px)"]), springConfig);
  const springYHowItWorksExit = useSpring(useTransform(howItWorksExitProgress, [0, 1], [0, -40]), springConfig);

  // Hero title parallax transformations (spring-smoothed)
  const xNutro = useSpring(useTransform(scrollY, [0, 800], [0, -180]), springConfig);
  const xBot = useSpring(useTransform(scrollY, [0, 800], [0, 180]), springConfig);
  const opacityTitle = useSpring(useTransform(scrollY, [0, 600], [1, 0]), springConfig);
  const scaleHero = useSpring(useTransform(scrollY, [0, 600], [1, 0.95]), springConfig);
  
  const yContent = useSpring(useTransform(scrollY, [0, 600], [0, 60]), springConfig);
  const opacityContent = useSpring(useTransform(scrollY, [0, 500], [1, 0]), springConfig);

  // Floating background shapes (Parallax)
  const yPlus = useSpring(useTransform(scrollY, [0, 1000], [0, -320]), springConfig);
  const rotatePlus = useSpring(useTransform(scrollY, [0, 1000], [0, 270]), springConfig);

  const ySquare = useSpring(useTransform(scrollY, [0, 1000], [0, -200]), springConfig);
  const rotateSquare = useSpring(useTransform(scrollY, [0, 1000], [0, -180]), springConfig);

  const yCircle = useSpring(useTransform(scrollY, [0, 1000], [0, -120]), springConfig);
  const scaleCircle = useSpring(useTransform(scrollY, [0, 1000], [1, 1.3]), springConfig);

  const yDash = useSpring(useTransform(scrollY, [0, 1000], [0, -380]), springConfig);
  ...
  return (
    <PageTransition className="relative w-full overflow-x-hidden bg-transparent">
      ...
      {/* Container 1 Wrapper (Hero Stack Page) */}
      <div ref={container1Ref} className="relative md:h-[145vh] h-auto z-10">
        <motion.div 
          style={{ 
            scale: isMobile ? undefined : springScaleHeroExit,
            opacity: isMobile ? undefined : springOpacityHeroExit,
            filter: isMobile ? undefined : springBlurHeroExit,
            y: isMobile ? undefined : springYHeroExit
          }}
          className="md:sticky md:top-0 md:h-screen h-auto w-full overflow-hidden flex flex-col justify-center px-6 pt-24 pb-12"
        >
          ...
          <div className="relative z-10 flex flex-col gap-12 max-w-7xl mx-auto w-full">
            <motion.div style={{ opacity: opacityTitle, scale: scaleHero }} className="flex flex-col">
              <motion.h1 
                style={{ 
                  x: xNutro,
      ...
      {/* Container 2 Wrapper (How It Works Stack Page) */}
      <div ref={container2Ref} className="relative md:h-[155vh] h-auto z-20">
        <motion.div 
          style={{ 
            scale: isMobile ? undefined : springScaleHowItWorksExit,
            opacity: isMobile ? undefined : springOpacityHowItWorksExit,
            filter: isMobile ? undefined : springBlurHowItWorksExit,
            y: isMobile ? undefined : springYHowItWorksExit
          }}
          className="md:sticky md:top-0 md:h-screen h-auto w-full overflow-hidden flex flex-col justify-between bg-[#E4E2DD] border-t-4 border-primary"
        >
          {/* Dynamic Marquee Section */}
          <ScrollMarquee />

          <div id="how-it-works-section" className="flex-grow flex flex-col justify-center py-20 px-6 max-w-7xl mx-auto w-full">
      ...
      {/* Container 3 Wrapper (Campaign Connect Stack Page - Slides on top of Container 2) */}
      <div ref={container3Ref} className="relative z-30 md:min-h-screen min-h-0 flex flex-col justify-center bg-[#D9D6D0] py-32 px-6 border-t-4 border-primary">
